...
| F1.03 | Điều phối drone, tạo mission | accept khóa order và giữ drone `IDLE → RESERVED` bằng compare-and-set có khóa DB `LockerOrderRepository.java:28-32`, `DroneOrderMaintenanceService.java:57-96`, `LockerService.java:1584-1596`; launch kiểm tra lại drone/pin/pad rồi `RESERVED → IN_FLIGHT` `DroneOrderMaintenanceService.java:143-176` | **DONE** (thủ công) | Không tự động gán |
| F1.04 | Nạp hàng tại điểm đi | accept tạo mission `AWAITING_LOADING`; `POST …/{id}/loading-confirmation` ghi trạm nguồn, người nạp, khối lượng, mã niêm phong và 3 mục checklist rồi mới chuyển `READY_TO_LAUNCH` (`DroneOrderMaintenanceService.java:86,101-140`); launch kiểm tra lại hồ sơ (`:157-160`); app có form xác nhận (`maintenance_home_page.dart:669-803`) | **DONE** | Giới hạn tải hiện dùng một cấu hình chung cho toàn đội, chưa theo model drone |
...
**Điểm:** DONE 4 × 1 + PARTIAL 6 × 0,5 = 7 / 10 = **70 %**. Ở STANDARD các mục F1.05–F1.08 không đạt được ⇒ tiến độ thực tế thấp hơn.
...
3. **DRONE_TECHNICIAN** `GET /api/drone-technician/drone-orders` → `POST …/{id}/accept {droneUnitId}` ⇒ khóa bản ghi drone và đổi nguyên tử `IDLE → RESERVED`, mission `AWAITING_LOADING`, stage `ACCEPTED`; đội viên đã nhận là người chịu trách nhiệm mission.
4. **Đội viên phụ trách** cân và nạp kiện, nhập mã niêm phong, xác nhận đúng kiện + đã cố định + khoang đã khóa qua `POST …/{id}/loading-confirmation` ⇒ mission `READY_TO_LAUNCH`. Quá tải cấu hình hoặc đội viên khác thao tác đều bị chặn.
5. **Đội viên phụ trách** `POST …/{id}/launch` ⇒ bắt buộc có hồ sơ nạp hàng, kiểm tra lại drone còn `RESERVED`, active, đủ pin và bãi đáp còn OK; đổi nguyên tử `RESERVED → IN_FLIGHT`, stage `LAUNCHING`.
6. **Bộ giả lập** (chỉ DEMO) mỗi 3 giây: `DEPARTED → EN_ROUTE → APPROACHING → ARRIVED` ⇒ mission `DEPOSITED`, order `STORING`, stage `READY_FOR_PICKUP`, cấp PIN, drone `IN_FLIGHT → IDLE` nếu trạng thái chưa bị đổi sang FAULT. **Ở STANDARD không có gì xảy ra sau bước 5.**
7. **Khách** xem timeline (poll 3 s), thấy PIN/QR khi đã PAID.
8. **Khách** nhập PIN ở kiosk ⇒ ô mở **nhưng đơn vẫn STORING**; hoặc bấm "hoàn tất" trong app ⇒ `COMPLETED`, nhả ô.
9. Không ai lấy ⇒ cron mỗi giờ: quá hạn + 24h ⇒ `EXPIRED`, nhả ô.
...
| **F1-G02** | **DONE · DEPLOYED 2026-09-20:** khóa order chống hai người nhận cùng đơn; accept chặn mission trùng và giữ drone `RESERVED` nguyên tử; launch tái kiểm tra drone/pin/pad; hủy nhả reservation; simulator chỉ nhả `IN_FLIGHT → IDLE`, không ghi đè FAULT; trạng thái vận hành không cho sửa tay | `LockerOrderRepository.java:28-32`, `DroneOrderMaintenanceService.java:57-129,155`, `DroneDeliverySimulator.java:96`, `LS/service/LockerService.java:1560-1596`; backend `633722d`, mobile `4688071` | F1.03 |
...
| **F1-G08** | **DONE LOCAL · CHƯA MERGE/DEPLOY:** mission `AWAITING_LOADING`; endpoint xác nhận trạm nguồn/người nạp/khối lượng/niêm phong/checklist; chặn quá tải, sai người phụ trách và launch thiếu hồ sơ; migration đưa mission cũ về chờ nạp; mobile có form vận hành | `OS/model/DroneMission.java:40-77`, `OS/service/DroneOrderMaintenanceService.java:101-176,317-333`, `V15__drone_loading_confirmation.sql`, `M/features/locker_ops/…:671-805` | F1.04 |
