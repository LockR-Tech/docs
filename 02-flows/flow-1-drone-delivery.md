...
| F1.03 | Điều phối drone, tạo mission | accept `DroneOrderMaintenanceService.java:47-85` (drone IDLE, pin > 20, pad OK); launch `:88-112`; app `M/…/maintenance_home_page.dart:589,779` | **DONE** (thủ công) | Không tự động gán; accept không giữ chỗ drone; accept lại được khi đang bay |
...
| F1.05 | Theo dõi hành trình gần thời gian thực | Chặng do `OS/service/DroneDeliverySimulator.java:37-90` (DEMO, 7 s/chặng); app poll 3 s `M/…/drone_delivery_providers.dart:35-48`; live map tắt `M/core/config/feature_flags.dart:47` | PARTIAL | Không telemetry thật; API trả missionId/droneCode/eta = null (`OrderService.java:1722`); `DroneDeliveryQueryService` không được gọi |
...
3. **MAINTENANCE** `GET /api/maintenance/drone-orders` → `POST …/{id}/accept {droneUnitId}` ⇒ mission `READY_TO_LAUNCH`, stage `ACCEPTED`.
4. **MAINTENANCE** `POST …/{id}/launch` ⇒ drone `IN_FLIGHT`, stage `LAUNCHING`.
5. **Bộ giả lập** (chỉ DEMO) mỗi 7 giây: `DEPARTED → EN_ROUTE → APPROACHING → ARRIVED` ⇒ mission `DEPOSITED`, order `STORING`, stage `READY_FOR_PICKUP`, cấp PIN, drone `IDLE`. **Ở STANDARD không có gì xảy ra sau bước 4.**
...
| **F1-G02** | Sửa guard mission: accept xét `deliveryStage` + mission; launch kiểm tra drone IDLE; giữ chỗ drone khi accept; simulator không ghi đè FAULT | `DroneOrderMaintenanceService.java`, `DroneDeliverySimulator.java`, `LS/service/LockerService.java:669` | F1.03 |
